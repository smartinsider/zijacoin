...
*Setup instructions for a gitian build of Zijacoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Zijacoin
...
- [Building Zijacoin](#building-zijacoin)
...
Clone the git repositories for zijacoin and gitian,
...
In [doc/release-process.md](release-process.md) in the zijacoin repository under 'Fetch and build inputs'.
...
To build Zijacoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the zijacoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zijacoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/coin-universe/zijacoin.git
...
For example:
```bash
URL=https://github.com/user/zijacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit zijacoin=${COMMIT} --url zijacoin=${URL} ../zijacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoin=${COMMIT} --url zijacoin=${URL} ../zijacoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/zijacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zijacoin-build.assert
```
...
[zijacoin/gitian.sigs](https://github.com/zijacoin-project/gitian.sigs/) repository, or
