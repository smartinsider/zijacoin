...
#ifndef ZIJA_ZZIJAWALLET_H
#define ZIJA_ZZIJAWALLET_H
...
public:
    CzZIJAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZZIJA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZZIJA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ZIJA_ZZIJAWALLET_H
